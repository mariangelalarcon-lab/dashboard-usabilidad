...
# 1. Configuración de página y Estilo (Fuente Philosopher)
st.set_page_config(page_title="Reporte de Usabilidad Holos", layout="wide")

st.markdown("""
    <link href="https://fonts.googleapis.com/css2?family=Philosopher:wght@400;700&display=swap" rel="stylesheet">
    <style>
        html, body, [class*="css"], .stMarkdown, h1, h2, h3, h4, .stText {
...
def load_data():
    ...
    # Función de limpieza con lógica específica para Cardif/Scotiabank
    def limpiar_pct(row):
        valor = row[c_usa]
        ...
        try:
            n = float(s)
            # CORRECCIÓN SOLICITADA: Solo para Cardif y Scotiabank
            if "cardif" in empresa or "scotiabank" in empresa:
                # Si viene como 56, queremos que sea 0.56% (0.0056)
                if n > 1.0: return n / 10000.0
                return n / 100.0
            
            # Para las demás empresas, lógica estándar
    ...
    df = df[df['Anio_Limpio'] > 2020].copy() # Elimina años como 1899
...
try:
    df = load_data()
    if df.empty:
        st.warning("No se detectaron archivos de datos.")
        st.stop()

    # --- ENCABEZADO ---
    col_logo, col_tit, col_g1, col_g2, col_g3 = st.columns([0.8, 1.5, 1, 1, 1])
    with col_logo:
    ...
    with st.sidebar:
        ...
        # Renombrar visualmente 'Mes total' a 'Acumulado'
        opciones_vista = ["Acumulado" if "total" in opt.lower() else opt for opt in opciones_sem]
        seleccion_vista = st.selectbox("Vista Temporal", opciones_vista)
        # Mapeo inverso para filtrar el DF
        semana_sel = "Mes total" if seleccion_vista == "Acumulado" else seleccion_vista

    # --- INDICADORES SUPERIORES (Gauges) ---
    def crear_gauge(anio, color):
        # El 2026 ahora sí jalará datos porque busca 'Mes total' (Acumulado)
        ...
        fig.update_layout(height=160, margin=dict(l=15, r=15, t=50, b=10), paper_bgcolor='rgba(0,0,0,0)')
        return fig
    ...
    if not df_f.empty:
        ...
        for a in sorted(anios_sel):
            ...
            if not df_a.empty:
                ...
                fig_main.add_trace(go.Bar(
                    ...
                    name=f"Real {a}", marker_color=colores.get(a),
        ...
        fig_main.update_layout(
            ...
            yaxis=dict(tickformat=".2%", range=[0, max(df_f['Usabilidad_Limpia'].max()*1.2, 0.05)]),
