import streamlit as st
import pandas as pd
import plotly.graph_objects as go

# 1. CONFIGURACIÓN INICIAL
st.set_page_config(page_title="Holos | BI Dashboard", layout="wide")

LINK_1 = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSWiXR7BLxwzX2wtD_uF59pvxtus8BL5iqgymKSh2-Llwt6smOJzR7ROUxICr57DA/pub?gid=1638907402&single=true&output=csv"
LINK_2 = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSWiXR7BLxwzX2wtD_uF59pvxtus8BL5iqgymKSh2-Llwt6smOJzR7ROUxICr57DA/pub?gid=1341962834&single=true&output=csv"

# Paleta Holos
SKY, LEAF, SEA, CORAL, BLACK = "#D1E9F6", "#F1FB8C", "#A9C1F5", "#FF9F86", "#000000"

@st.cache_data(ttl=10) # TTL bajo para que veas tus cambios del Excel casi al instante
def cargar_datos():
    try:
        df1 = pd.read_csv(LINK_1)
        df2 = pd.read_csv(LINK_2)
        df = pd.concat([df1, df2], ignore_index=True)
        df.columns = [str(c).strip() for c in df.columns]
        
        # Mapeo de columnas
        c_emp = next((c for c in df.columns if 'Empresa' in c), df.columns[0])
        c_sem = next((c for c in df.columns if 'Semana' in c), df.columns[1])
        c_usa = next((c for c in df.columns if '%' in c or 'Usabilidad' in c), df.columns[7])
        c_mes = next((c for c in df.columns if 'Mes' in c and 'inicio' not in c.lower()), df.columns[8])
        c_ani = next((c for c in df.columns if 'Año' in c or 'Anio' in c), df.columns[11])

        # Limpieza
        df['Anio_V'] = pd.to_numeric(df[c_ani], errors='coerce').fillna(0).astype(int)
        df['Mes_V'] = pd.to_numeric(df[c_mes], errors='coerce').fillna(0).astype(int)
        df['Empresa_V'] = df[c_emp].astype(str).str.strip()
        df['Semana_V'] = df[c_sem].astype(str).str.strip()
        
        def limpiar_pct(x):
            try:
                v = str(x).replace('%', '').replace(',', '.').strip()
                n = float(v)
                return n / 100.0 if n > 1.1 else n
            except: return 0.0
            
        df['Usabilidad_V'] = df[c_usa].apply(limpiar_pct)
        # Filtro de seguridad años reales
        return df[(df['Anio_V'] >= 2024) & (df['Anio_V'] <= 2026)].copy()
    except: return pd.DataFrame()

df = cargar_datos()

if not df.empty:
    # --- SIDEBAR ---
    with st.sidebar:
        st.header("📊 Panel de Control")
        
        # NUEVO: Selector de vista para tu reunión
        tipo_vista = st.radio("Nivel de Detalle:", ["Vista Ejecutiva (Cierres)", "Vista Operativa (Semanas)"])
        
        st.markdown("---")
        emp_list = sorted([e for e in df['Empresa_V'].unique() if e != 'nan'])
        empresa_sel = st.selectbox("Empresa", ["Todas las Empresas"] + emp_list)
        
        anios_sel = st.multiselect("Años", sorted(df['Anio_V'].unique(), reverse=True), default=[2025, 2026])
        meses_map = {1:'Ene', 2:'Feb', 3:'Mar', 4:'Abr', 5:'May', 6:'Jun', 7:'Jul', 8:'Ago', 9:'Set', 10:'Oct', 11:'Nov', 12:'Dic'}
        meses_sel = st.multiselect("Meses", sorted(meses_map.keys()), default=list(meses_map.keys()), format_func=lambda x: meses_map[x])

    # --- FILTRADO INTELIGENTE ---
    df_f = df[(df['Anio_V'].isin(anios_sel)) & (df['Mes_V'].isin(meses_sel))]
    
    if empresa_sel != "Todas las Empresas":
        df_f = df_f[df_f['Empresa_V'] == empresa_sel]

    # Aquí aplicamos el filtro de "Semana" según lo que quieras ver
    if tipo_vista == "Vista Ejecutiva (Cierres)":
        df_vis = df_f[df_f['Semana_V'].str.contains('total|Total', na=False)]
    else:
        # En modo operativo mostramos todo para ver el progreso de Febrero
        df_vis = df_f.copy()

    # --- VISUALIZACIÓN ---
    st.title(f"Reporte de Usabilidad: {empresa_sel}")
    st.info(f"Mostrando: {tipo_vista}")

    # 1. MÉTRICAS (GAUGES)
    anios_act = sorted(df_vis['Anio_V'].unique())
    col_anios = {2024: LEAF, 2025: CORAL, 2026: SEA}
    
    if not df_vis.empty:
        cols = st.columns(len(anios_act))
        for i, a in enumerate(anios_act):
            with cols[i]:
                # Promedio de los valores visibles
                media = df_vis[df_vis['Anio_V'] == a]['Usabilidad_V'].mean()
                
                fig = go.Figure(go.Indicator(
                    mode="gauge+number", value=media*100,
                    number={'suffix': "%", 'valueformat': '.2f'},
                    title={'text': f"Media {a}"},
                    gauge={'axis': {'range': [0, 100]}, 'bar': {'color': BLACK},
                           'steps': [{'range': [0, 100], 'color': col_anios.get(a, SKY)}]}
                ))
                fig.update_layout(height=260, margin=dict(l=20,r=20,t=40,b=20))
                st.plotly_chart(fig, use_container_width=True, key=f"g_{a}")

    # 2. GRÁFICA DE PROGRESO (MEJORADA)
    st.subheader(f"📈 Curva de Progreso - {tipo_vista}")
    
    if not df_vis.empty:
        # Agrupamos por mes y semana para que la línea siga el orden correcto
        df_chart = df_vis.groupby(['Anio_V', 'Mes_V', 'Semana_V'])['Usabilidad_V'].mean().reset_index()
        
        fig_l = go.Figure()
        for a in anios_act:
            df_p = df_chart[df_chart['Anio_V'] == a].sort_values(['Mes_V', 'Semana_V'])
            
            # Eje X que combina Mes y Semana para ver el avance
            x_labels = [f"{meses_map.get(m)}-{s}" for m, s in zip(df_p['Mes_V'], df_p['Semana_V'])]
            
            fig_l.add_trace(go.Scatter(
                x=x_labels, y=df_p['Usabilidad_V'],
                name=f"Año {a}", mode='lines+markers+text',
                text=[f"{v:.1%}" for v in df_p['Usabilidad_V']], textposition="top center",
                line=dict(color=col_anios.get(a, BLACK), width=3)
            ))
        
        fig_l.update_layout(yaxis=dict(tickformat=".0%", range=[0, 1.1]), height=450)
        st.plotly_chart(fig_l, use_container_width=True)

    # 3. TABLA DE APOYO PARA LA REUNIÓN
    with st.expander("📝 Ver detalle de datos (Auditoría de Febrero)"):
        st.write("Datos actuales filtrados:")
        st.dataframe(df_vis[['Empresa_V', 'Anio_V', 'Mes_V', 'Semana_V', 'Usabilidad_V']].sort_values(['Anio_V', 'Mes_V', 'Semana_V'], ascending=False))

else:
    st.error("No se detectan datos. Por favor, revisa que el Excel esté actualizado y presiona 'C' para limpiar el caché.")
